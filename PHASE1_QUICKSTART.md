...
3. 씬 하나 생성 (이름 예: `Evergreen` — 나중에 진짜 지역 이동 기능을 붙일 때 재사용하도록 미리 이 이름으로)
...
1. `GameObject > 2D Object > Sprites > Square` 등으로 바닥 느낌의 큰 배경 오브젝트 하나 배치 (Sprite Renderer의
   Color를 초원/평야 느낌으로 변경하면 에버그린다운 느낌)
...
지금 만든 씬 이름을 `Evergreen`으로 미리 맞춰뒀다면, 나중에 `Planet_Evergreen` 데이터 에셋만 추가로 만들어서
그대로 이어붙일 수 있습니다.
