...
🎮 **[지금 바로 플레이하기](https://kri2126.github.io/everplanet/web/evergreen-exploration.html)** — 설치 없이 브라우저에서 바로 실행됩니다.
...
- `web/evergreen-exploration.html` — 현재 플레이 가능한 웹 프로토타입(HTML5 Canvas, 바닐라 JS).
...
1단계(이동·환경·카메라)까지 완성. 이후 전투 → 성장 시스템 → 에버그린 내 콘텐츠(지역·몬스터·
